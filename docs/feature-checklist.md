...
| About page (detailed) | :white_check_mark: | Low | Rewritten with real motivation/stats/citations (2026-09-18) |
| Custom 404 page | :x: | Low | Not detected by this script; manually verified: no `overrides/404.html`, no `custom_dir` in mkdocs.yml. Mascot images exist, so `custom-404-page.md` could add one quickly |
| Simple feedback widget (👍/👎) | :x: | Low | Not detected by this script; manually verified: no feedback config in mkdocs.yml. See `mkdocs-features.md` feature 15 |
...
| Feature | Status | Effort | Notes |
|---------|--------|--------|-------|
| Glossary | :white_check_mark: | Medium | 561 terms with ISO 11179 compliant definitions |
...
| Per-chapter references | :white_check_mark: | Medium | 2 chapter-level references.md files |
...
| Document status indicators | :construction: | Low | CSS and mkdocs.yml config (scaffold/built/approved) are in place, but no page currently sets `status:` in its frontmatter — wired but unused |
...
| Per-chapter quizzes | :white_check_mark: | High | 2 quiz files with questions aligned to learning objectives |
...
| Feature | Status | Effort | Notes |
|---------|--------|--------|-------|
| Chapter content | :construction: | Very High | Only 2 of 27 chapters have full content (Ch 1-2); the other 25 have finished outlines but still say "TODO: Generate Chapter Content" |
